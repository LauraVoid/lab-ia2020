...
class TicTacToeGame():
  ...
  def is_over(self): # TODO: Finish this function by adding checks for a winning game (rows, columns, diagonals)
     if(self.turn== _MACHINE): symbol = _PLAYER_SYMBOL
    elif(self.turn== _PLAYER): symbol = _MACHINE_SYMBOL
  ...
  def print_result(self): # TODO: Finish this function in order to print the result based on the *winner*
    if(self.turn == _MACHINE and self.is_over()):print("The machine is the winner")
    elif(self.turn== _PLAYER and self.is_over()):print("Player is the winner")
